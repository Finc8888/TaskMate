...
document.getElementById("btn-truncate-tasks")?.addEventListener("click",async (e) =>{
    fetch("http://taskassistant.herokuapp.com/truncate_tasks", {method:'post',body:''});
...
const buildTaskList = async ( ) => {
    const list = await taskList.getTaskList('http://taskassistant.herokuapp.com/get_all_tasks');
